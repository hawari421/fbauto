...
print(GREEN + '\x1b[38;5;22m•'*60 + RESET)
print(GREEN + '\x1b[38;5;22m•'*60 + RESET)
print(GREEN + '\x1b[38;5;208m⇼'*60 + RESET)
...
# Mail.tm account creation with OTP
def create_mail_tm_account(proxy=None):
    ...
    if mail_domains:
        ...
        otp = generate_otp()  # Generate OTP for the account
...
def register_facebook_account(email, password, first_name, last_name, birthday, otp, proxy=None):
    ...
    # Simulating OTP verification before Facebook registration
    if otp:
        print(f"{GREEN}OTP: {otp} - Verifying...{RESET}")
        # Add OTP verification logic here (Optional, can just print it for demo purposes)
        print(f"{GREEN}OTP Verified: {otp}{RESET}")
    ...
    if reg and 'new_user_id' in reg:
        ...
        # Display result with green background in terminal
        print(f"""{GREEN}
-----------GENERATED-----------
EMAIL : {email}
ID : {id}
